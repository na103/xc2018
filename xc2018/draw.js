...
var rowInfo = {
 ...
 // "row.A.io2": [226, 60], // My invention, I/O above CLB
 "row.A.local.0": [211, 2],
 "CLK.AA.O": [208, 22],
 "row.A.long.2": [207, 26],
 ...
 "row.A.k": [190, 92], // My invention, input k to CLB    aaaaa
 "row.A.y": [189, 96], // My invention, input d to CLB

 "row.K.local.0": [17, 748],
 "row.K.io1": [16, 752], // My invention
 "row.K.io2": [15, 756], // My invention
 "row.K.io3": [14, 760], // My invention
 "row.K.io4": [13, 764], // My invention
 "row.K.long.1": [12, 768],
 "row.K.local.1": [10, 774],
 "row.K.local.2": [9, 778],
 "row.K.local.3": [7, 782],
 "row.K.local.4": [6, 786],
 "row.K.long.2": [4, 790],
 "row.K.clk": [3, 796], // My invention
 "row.K.local.5": [0, 816],
...
// This makes assigning iobs to columns easier.
 ...
 "col.K.clbw2": [203, 746],
 "col.K.clbw3": [204, 750],
 "col.K.io1": [205, 754], // My invention, the column used just for I/O pins
 "col.K.io2": [206, 758], // My invention, the column used just for I/O pins
 "col.K.local.0": [207, 762],
 "col.K.io3": [208, 766], // My invention, the column used just for I/O pins
 "col.K.long.1": [209, 770],
 "col.K.long.2": [210, 774],
 "col.K.local.1": [212, 780],
 "col.K.local.2": [213, 784],
 "col.K.local.3": [215, 788],
 "col.K.local.4": [216, 792],
 "col.K.long.3": [218, 796],
 "col.K.local.5": [221, 816],
...
function initNames() {
  // Generate formulaic row and column names (B through J)
  for (var i = 1; i < 10; i++) {
    ...
    rowInfo['row.' + name + '.k'] = [rstart - 6, 164 + 72 * (i-1)];  //aaaaaaaaaaaa
  ...
  // Bit position starts for the tiles A through K. Note there is I/O before A and buffers between C-D and F-G.
  var xTileStarts = [3, 21, 39, 57, 77, 95, 113, 133, 151, 169, 187];

  /**
   * Take a bit index and return the tile A-K, along with starting bitstream index.
   */
  ...
  var yTileStarts = [1, 9, 17, 25, 34, 42, 50, 59, 67, 75, 83];
  ...
  function getSwitchCoords(name) {
    const m = name.match(/([A-K][A-K])\.8\.(\d)\.(\d)$/);
    if (m == undefined) {
      throw "Bad name " + name;
    }
    const tilename = m[1];
...
const SCALE = 2;
  ...
  function drawLayout(ctx) {
    ...
    const HEIGHT = 819 * SCALE;
    const WIDTH = 819 * SCALE;
