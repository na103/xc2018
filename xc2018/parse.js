...
// Raw bitstream, sequence of 196*87 0/1 values
...
class SwitchDecoders {
  ...
  function nameToG(str) {
    ...
    const m = str.match(/([A-K][A-K])\.8\.(\d)\.(\d)$/);
    if (str.match(/([A-K][A-K])\.8\.(\d)\.(\d)$/)) {
...
const BITTYPE = Object.freeze({lut: 1, clb: 2, pip: 3, switch: 5, iob: 6, bidi: 7, other: 8, unused: 9});
  ...
  class TileDecode {
    constructor(x, y) {
      ...
      this.pins = [];
      if (x < 10 && y < 10) {
        this.clb = new Clb(x, y, [x * 72 + 78, y * 72 + 68], [x * 19, y * 20], this.bitPt); // origine top,left x px e y px. passo 72px - 19x20 quadretti nella bit grid
	  } else {
        this.clb = null;
      }
      ...
      if (this.type == TILE.ul) {
        ...
        var pips = [
          ["col.K.local.4:row.A.long.2", [152, 4]], ["col.K.long.3:row.A.long.2", [153, 4]],
          ["col.K.local.0:row.A.local.1", [-1, -1]], ["col.K.local.4:row.A.local.1", [151, 2]], ["col.K.long.3:row.A.local.1", [152, 2]],
          ["col.K.local.0:row.A.local.2", [-1, -1]], ["col.K.local.3:row.A.local.2", [155, 4]],
          ["col.K.local.0:row.A.local.3", [-1, -1]], ["col.K.local.2:row.A.local.3", [157, 4]],
          ["col.K.local.1:row.A.local.4", [156, 4]], ["col.K.local.0:row.A.local.4", [-1, -1]],
          ["col.K.local.0:row.A.long.3", [-1, -1]], ["col.K.long.1:row.A.long.3", [154, 2]], ["col.K.long.2:row.A.long.3", [153, 2]],
        ...
        var pips = [
          ["col.K.long.2:row.ROW.local.1", [159, 11]],
          ["col.K.long.1:row.ROW.local.3", [153, 11]],
          ["col.K.long.2:row.ROW.local.4", [153, 12]],
          ["col.K.long.1:row.ROW.local.5", [154, 12]],
          ["col.K.long.1:row.ROW.long.1", [154, 11]], ["col.K.long.2:row.ROW.long.1", [158, 11]], ["col.K.local.1:row.ROW.long.1", [155, 11]], ["col.K.local.4:row.ROW.long.1", [151, 11]], ["col.K.long.3:row.ROW.long.1", [152, 11]]];
        this.switch1 = new Switch(this, 1);
        this.switch2 = new Switch(this, 2);
        pips.forEach(pip => this.pips.push(new Pip(rename(pip[0]), [pip[1][0], pip[1][1] + yoffset])));
        ...
        // bottom right
        var pips = [
          ["col.K.long.1:row.K.long.1", [155, 67]], ["col.K.long.2:row.K.long.1", [158, 67]],
          ["col.K.local.1:row.K.local.1", [156, 67]],
          ["col.K.local.2:row.K.local.2", [157, 67]],
          ["col.K.local.3:row.K.local.3", [154, 67]],
          ["col.K.local.4:row.K.local.4", [151, 68]], ["col.K.long.3:row.K.local.4", [152, 67]],
          ["col.K.local.4:row.K.long.2", [151, 67]], ["col.K.long.3:row.K.long.2", [153, 67]]];
        pips.forEach(pip => this.pips.push(new Pip(rename(pip[0]), pip[1])));
      }
    }
